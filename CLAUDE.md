...
- **`Common/GlobalItems/ProjectileManager.cs`** — **Player→enemy only.** Data-only: snapshots weapon affixes in `OnSpawn` (no `ModifyHitNPC` override here — conditional damage bonuses (Nearby/Distant/LowHp/FullHp) are applied by `ARPGCharacterSystem.Common.Players.OutgoingHitPlayer.ApplyHitTimePoolAdditions`, which reads this manager's `Affixes` and `WeaponBaseDamage`). `OnSpawn` handles three sources: `EntitySource_ItemUse` (player weapons), `EntitySource_Parent` where parent is a player projectile (sentry/minion sub-shots always inherit the parent's `WeaponBaseDamage`, and its affixes when it has any), and everything else is left empty. Elemental math is handled by `ARPGCharacterSystem.Common.Players.OutgoingHitPlayer.ModifyHitNPCWithProj`, which reads the snapshot affixes from this manager. **ARPGCharacterSystem's on-hit spawn system fills `Affixes`/`WeaponBaseDamage` for projectiles it spawns itself** (`SpawnExecutor.ApplySpawnDataTo`, called on the line after `Projectile.NewProjectile`, since `OnSpawn`'s `EntitySource_ItemUse`/`EntitySource_Parent` cases don't recognize that spawn's `IEntitySource`) — nothing in ItemSystem may read these fields during `OnSpawn` itself, or a self-spawned projectile will miss them.
...
**Exception — pipeline-applied affixes:** All player-stat affixes that previously had switch cases in `ArmorManager.UpdateEquip` or `AccessoryManager.UpdateAccessory` (FlatLifeIncrease, FlatManaIncrease, LifeRegeneration, IncreasedManaRegeneration, the four `Percentage*DamageIncrease` lines, FlatCritChance, ManaEfficiency, accessory `FlatDefenseIncrease`) — plus elemental resistances and penetration — are now applied by `ARPGCharacterSystem.Common.Stats.Sources.EquipmentStatSource.Dispatch`. The attribute-point suffixes (`Constitution` … `Resilience`, armor only, added 2026-09-25) are declared by one loop in `BuildRegistry` because they share a tier table; CharacterSystem's `EquipmentStatSource` maps each to the same-named `AttributeKind`. Do NOT add switch cases for player-stat affixes in `ArmorManager` / `AccessoryManager`. Per-hit damage affixes (`GainPercentAsX`, `IncreasedXDamage`) still flow through `ElementalDamageCalculator.ApplyToHit` via `GetMagnitude`.
