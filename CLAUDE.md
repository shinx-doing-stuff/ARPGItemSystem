...
**Exception — pipeline-applied affixes:** All player-stat affixes that previously had switch cases in `ArmorManager.UpdateEquip` or `AccessoryManager.UpdateAccessory` (FlatLifeIncrease, FlatManaIncrease, LifeRegeneration, IncreasedManaRegeneration, the four `Percentage*DamageIncrease` lines, FlatCritChance, ManaEfficiency, accessory `FlatDefenseIncrease`) — plus elemental resistances and penetration — are now applied by `ARPGCharacterSystem.Common.Stats.Sources.EquipmentStatSource.Dispatch`. Do NOT add switch cases for player-stat affixes in `ArmorManager` / `AccessoryManager`. Per-hit damage affixes (`GainPercentAsX`, `IncreasedXDamage`) still flow through `ElementalDamageCalculator.ApplyToHit` via `GetMagnitude`.
